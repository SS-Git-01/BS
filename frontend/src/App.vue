    ...
    <div v-else class="main-content">
      ...
      <van-sticky :offset-top="46">
        <van-search 
          v-model="searchQuery" 
          show-action 
          placeholder="🔍 搜地点 / AI 标签 (如: mask, stage)" 
          @search="onSearch"
          @clear="onSearch"
        >
          <template #action>
      ...
      <van-grid :column-num="2" gutter="10" class="image-grid">
        <van-grid-item v-for="(img, index) in images" :key="img.id" class="grid-item-wrapper">
          ...
          <div class="image-info">
            ...
            <div class="tags-row" v-if="img.ai_tags && img.ai_tags.length > 0">
              <span v-for="(tag, tIndex) in img.ai_tags.slice(0, 3)" :key="tIndex" class="ai-tag">
                🤖 {{ tag.label }}
              </span>
            </div>
          </div>
...
.delete-btn { position: absolute; top: 5px; right: 5px; width: 24px; height: 24px; background: rgba(0, 0, 0, 0.5); border-radius: 4px; color: #fff; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 10; transition: all 0.2s; }
